...
export default defineConfig({
    ...
    build: {
        ...
        rollupOptions: {
            ...
            {
                if(source.includes('task-card.ts') || source.includes('task-card.js') || source.includes('task-card.min.js')) { return true; }
            },
            output: {
    ...
    plugins: [{
        ...
        transform(code, id) {
            const libraryMatch = code.match(/['"].*?task-card.*?["']/g);
            if(libraryMatch == null) { return null; }
            // console.log(libraryMatch);
            const result = code.replace(libraryMatch[0], '"../libs/task-card.min.js"');
            // console.log(result);
            return {
                code: result,
